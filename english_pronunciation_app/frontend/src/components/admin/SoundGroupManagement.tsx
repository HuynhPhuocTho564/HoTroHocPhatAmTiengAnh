  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-neutral-900">Quản lý Sound Group</h2>
          <p className="mt-1 text-sm text-neutral-600">{items.length} nhóm âm</p>
        </div>
      ...
      <input type="search" placeholder="Tìm theo tên..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-lg border border-neutral-300 px-4 py-2 text-sm" />
        ...
        <Card>
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div><label className="mb-1 block text-sm font-semibold">Chủ đề</label><select value={form.topicId} onChange={(e) => setForm({ ...form, topicId: e.target.value })} className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm"><option value="">-- Không --</option>{topics.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
