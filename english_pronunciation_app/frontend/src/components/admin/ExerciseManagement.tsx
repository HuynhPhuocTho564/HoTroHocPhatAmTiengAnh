  ...
  return (
    <label className="block" htmlFor={id}>
      ...
      <input
        ...
        className="min-h-11 w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm text-neutral-900 focus:outline-none focus-visible:ring-4 focus-visible:ring-primary-500"
      />
    </label>
  );
}
    ...
    <Card>
      <div className="p-6">
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-lg font-bold text-neutral-900">Quản lý bài tập</h2>
            <p className="mt-1 text-sm text-neutral-600">Tổng số: {items.length} bài tập</p>
          </div>
        </div>
        ...
        <div className="mb-6 rounded-lg border border-neutral-200 bg-neutral-50 p-4">
          <form onSubmit={submitForm} className="space-y-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h3 className="font-bold text-neutral-900">{editingId ? "Sửa bài tập" : "Tạo bài tập mới"}</h3>
            ...
            {!referenceDataReady && (
              <div className="rounded-lg border border-warning-200 bg-warning-50 p-3 text-sm font-medium text-warning-800" role="alert">
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              ...
              <TextField
                id="exercise-time-limit"
                label="Thời gian giới hạn (giây)"
                type="number"
                value={form.timeLimit}
                onChange={(value) => updateForm("timeLimit", value)}
              />
            </div>

            <label className="block" htmlFor="exercise-description">
              <span className="mb-1 block text-sm font-semibold text-neutral-700">Mô tả</span>
              <textarea
                id="exercise-description"
                value={form.description}
                onChange={(event) => updateForm("description", event.target.value)}
                rows={3}
                className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm text-neutral-900 focus:outline-none focus-visible:ring-4 focus-visible:ring-primary-500"
              />
            </label>
          ...
          {filters.map((status) => (
            <button
              ...
              className={`min-h-11 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-bold transition-colors focus:outline-none focus-visible:ring-4 focus-visible:ring-primary-500 ${
                filterStatus === status ? "bg-primary-600 text-white" : "bg-neutral-100 text-neutral-700 hover:bg-neutral-200"
              }`}
            >
        ...
        <div className="mb-6 rounded-lg border border-neutral-200 bg-white p-4">
          ...
          {questionExercise && (
            <p className="mb-4 text-sm text-neutral-600">
              Bài đang chọn: <span className="font-bold text-neutral-900">{questionExercise.name}</span>
            </p>
          )}

          {!questionTypeReady && (
            <div className="mb-4 rounded-lg border border-warning-200 bg-warning-50 p-3 text-sm font-medium text-warning-800" role="alert">
          ...
          <form onSubmit={submitQuestionForm} className="mb-6 space-y-4 rounded-lg border border-neutral-200 bg-neutral-50 p-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h3 className="font-bold text-neutral-900">{editingQuestionId ? "Sửa câu hỏi" : "Thêm câu hỏi"}</h3>
                <p className="mt-1 text-sm text-neutral-600">Mỗi dòng là một lựa chọn. Bài nói có thể để trống phần lựa chọn.</p>
              </div>
              {editingQuestionId && (
            ...
            <label className="block" htmlFor="question-content">
              <span className="mb-1 block text-sm font-semibold text-neutral-700">Nội dung câu hỏi</span>
              <textarea
                id="question-content"
                required
            ...
            <label className="block" htmlFor="question-options">
              <span className="mb-1 block text-sm font-semibold text-neutral-700">Lựa chọn trả lời</span>
              <textarea
                id="question-options"
              ...
              {questions.map((question) => (
                <article key={question.id} className="rounded-lg border border-neutral-200 p-4">
                  <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <h4 className="font-bold text-neutral-900">{question.name || question.content.slice(0, 80)}</h4>
                      <p className="mt-1 text-sm text-neutral-600">
                  ...
                  <p className="mb-3 whitespace-pre-wrap rounded-lg bg-neutral-50 p-3 text-sm text-neutral-700">{question.content}</p>
                  {question.options.length > 0 && (
                    <ul className="mb-3 flex flex-wrap gap-2">
                      {question.options.map((option) => (
                        <li key={option.id} className="rounded-full border border-neutral-200 px-3 py-1 text-xs font-semibold text-neutral-700">
                          {option.content}
                        </li>
                      ))}
                    </ul>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {filteredExercises.map((exercise) => (
            <article key={exercise.id} className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
                <div className="mb-3 flex items-start justify-between gap-3">
                  <h3 className="flex-1 font-semibold text-neutral-900">{exercise.name}</h3>
                  <Badge variant={statusVariant(exercise.status)} size="sm">
                    {exercise.status}
                  </Badge>
                </div>

                {exercise.description && <p className="mb-4 text-sm text-neutral-600">{exercise.description}</p>}

                <dl className="mb-4 grid grid-cols-2 gap-3 text-sm">
                  <div className="rounded-lg bg-neutral-50 p-3">
                    <dt className="text-xs text-neutral-500">Chủ đề</dt>
                    <dd className="font-semibold text-neutral-800">{exercise.topic}</dd>
                  </div>
                  <div className="rounded-lg bg-neutral-50 p-3">
                    <dt className="text-xs text-neutral-500">Cấp độ</dt>
                    <dd className="font-semibold text-neutral-800">{exercise.level}</dd>
                  </div>
                  <div className="rounded-lg bg-neutral-50 p-3">
                    <dt className="text-xs text-neutral-500">Câu hỏi đang dùng</dt>
                    <dd className="font-semibold text-neutral-800">{exercise.questionCount}</dd>
                  </div>
                  <div className="rounded-lg bg-neutral-50 p-3">
                    <dt className="text-xs text-neutral-500">Lượt làm</dt>
                    <dd className="font-semibold text-neutral-800">{exercise.attemptCount}</dd>
                  </div>
                </dl>

                <div className="mb-4 text-xs text-neutral-500">
                  <p>Map: {exercise.map}</p>
                  <p>Giới hạn thời gian: {exercise.timeLimit ? `${exercise.timeLimit}s` : "Không giới hạn"}</p>
                </div>
