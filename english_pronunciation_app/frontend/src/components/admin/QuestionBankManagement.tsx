  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-neutral-900">Quản lý Question Bank</h2>
          <p className="mt-1 text-sm text-neutral-600">{items.length} câu hỏi</p>
        </div>
      ...
      <input type="search" placeholder="Tìm theo answer hoặc prompt..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-lg border border-neutral-300 px-4 py-2 text-sm" />
        ...
        <Card>
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div><label className="mb-1 block text-sm font-semibold">Answer</label><input type="text" value={form.answer} onChange={(e) => setForm({ ...form, answer: e.target.value })} className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm" /></div>
              <div><label className="mb-1 block text-sm font-semibold">Question Type</label><select value={form.questionTypeId} onChange={(e) => setForm({ ...form, questionTypeId: e.target.value })} className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm"><option value="">-- Chọn --</option>{questionTypes.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
              <div><label className="mb-1 block text-sm font-semibold">Sound Group</label><select value={form.soundGroupId} onChange={(e) => setForm({ ...form, soundGroupId: e.target.value })} className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm"><option value="">-- Không --</option>{soundGroups.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
              ...
              <div className="md:col-span-2"><label className="mb-1 block text-sm font-semibold">Prompt</label><input type="text" value={form.prompt} onChange={(e) => setForm({ ...form, prompt: e.target.value })} className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm" /></div>
            </div>
