...
export default function TopicLevelMapManagement({ topics, levels, maps }: TopicLevelMapManagementProps) {
  ...
  return (
    <Card>
      <div className="p-6">
        <div className="mb-6">
          <h2 className="text-lg font-bold text-neutral-900">Quản lý chủ đề, cấp độ và learning map</h2>
          <p className="mt-1 text-sm text-neutral-600">Đây là dữ liệu nền để tạo bài tập và sắp xếp lộ trình học.</p>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`min-h-11 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-bold transition-colors focus:outline-none focus-visible:ring-4 focus-visible:ring-primary-500 ${
                activeKind === tab.id ? "bg-primary-600 text-white" : "bg-neutral-100 text-neutral-700 hover:bg-neutral-200"
              }`}
            >
              {tab.label}
            </button>
        ...
        <form onSubmit={submitForm} className="mb-6 space-y-4 rounded-lg border border-neutral-200 bg-neutral-50 p-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h3 className="font-bold text-neutral-900">{editingId ? `Sửa ${label}` : `Tạo ${label}`}</h3>
          ...
          {message && (
            <div
              className={`rounded-lg border p-3 text-sm font-medium ${
                message.type === "success" ? "border-success-200 bg-success-50 text-success-800" : "border-error-200 bg-error-50 text-error-800"
              }`}
          ...
          <label className="block" htmlFor="admin-base-details">
            <span className="mb-1 block text-sm font-semibold text-neutral-700">{detailLabel}</span>
            <textarea
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {mapItems.map((item) => (
              <article key={item.id} className="rounded-lg border border-neutral-200 bg-white p-4 shadow-sm">
                ...
                {item.requirement && <p className="mt-2 text-sm text-neutral-600">{item.requirement}</p>}
                <p className="mt-3 text-xs text-neutral-500">
