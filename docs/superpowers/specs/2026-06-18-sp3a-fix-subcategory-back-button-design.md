...
| (CĐ2/3/4 để sau, null tạm) | null | — |

Catalog `SoundGroupDefinition` thêm field `subcategory?: string`. Seed gán.
...
- Nếu có subcategory → render 2 mục "Nguyên âm đơn" / "Nguyên âm đôi", mỗi mục là 1 tiêu đề + grid nhóm âm.
- Nếu null → giữ hiển thị phẳng (cho CĐ2/3/4 sau).
...
- Test catalog: thêm check `subcategory` — 6 nhóm "Nguyên âm đơn" + 4 nhóm "Nguyên âm đôi" trong CĐ1.
